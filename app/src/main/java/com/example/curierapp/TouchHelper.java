...
//                            List<Address> updatedList = db.mainDAO().getAll();
        ...
        } else { //Если свайп был в левую сторону — запускаем метод editItems(...), а это мы уже знаем: откроется диалог с редактированием задачи.
            AlertDialog.Builder builder = new AlertDialog.Builder(adapter.getContext()); //Открываем AlertDialog с вопросом.
...
// Swipe влево (например, редактирование)
        new RecyclerViewSwipeDecorator.Builder(c, recyclerView, viewHolder, dX, dY, actionState, isCurrentlyActive)
                ...
                .addSwipeLeftActionIcon(R.drawable.edit_icon) // рисует иконку поверх этого фона (например, иконку редактирования
                .create()
                .decorate();
